...
const analyzeCode = async (req, res) => {
  try {
    ...
    if (!code && !repo_url) {
      return res.status(400).json({ error: 'Either code or repo_url must be provided' });
    }
...
const validateAnalysis = [
  body('code').optional().isLength({ min: 1 }).withMessage('Code cannot be empty if provided'),
  body('repo_url').optional().isURL().withMessage('Repository URL must be valid')
];
